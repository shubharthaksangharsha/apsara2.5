...
import { Menu, Trash2, UserIcon } from 'lucide-react';
        ...
        `}>
          ...
          <ul className="space-y-1 pb-2 overflow-y-auto custom-scrollbar flex-grow"> {/* Added overflow-y-auto, custom-scrollbar, and flex-grow */}
            {convos && convos.map(c => (
              <li
                key={c.id}
                className={`relative px-3 py-2 rounded-md cursor-pointer transition-all duration-150 ease-in-out flex justify-between items-center group/item ${ // Use different group name for item actions if needed
                  c.id === activeConvoId
              ...
              >
                ...
                <button
                  ...
                  className="p-1 text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 flex-shrink-0 opacity-0 group-hover/item:opacity-100 focus:opacity-100 transition-all duration-150 ease-in-out"
