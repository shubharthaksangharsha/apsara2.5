...
import { X, AlertTriangle, CheckCircle, Loader2, Image as ImageIcon } from 'lucide-react'; // Added Image as ImageIcon
...
export default function ImagePreviewItem({ file, onRemove, status }) {
  ...
  React.useEffect(() => {
    ...
    if (file instanceof File) {
      ...
      // For now, if it's not a File object, we rely on the initial File object having created a blob URL.
      // This part might need more robust handling if we expect to show previews for files that were never File objects on the client.
      // However, selectedImagesForPrompt should always start with File objects.
  ...
  } else if (status === 'success' && file.id) { 
    statusIndicator = (
  ...
  return (
    ...
    >
      ...
      {(status !== 'uploading') && ( // Simpler condition: Don't show remove if actively uploading
        <button
          type="button"
          onClick={onRemove}
