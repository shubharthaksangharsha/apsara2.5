...
import { LS_KEYS, DEFAULT_TEMPERATURE, DEFAULT_MAX_OUTPUT_TOKENS, 
         DEFAULT_THINKING_ENABLED, DEFAULT_THINKING_BUDGET, DEFAULT_FUNCTION_CALLING_MODE } from './constants';
...
export function useAppSettings(initialSystemInstruction) {
  ...
  // Function calling / Tool selection state
  const [enableFunctionCalling, setEnableFunctionCalling] = useState(() => 
    loadFromStorage(LS_KEYS.ENABLE_FUNCTION_CALLING, false, val => val === 'true'));
  const [selectedTools, setSelectedTools] = useState(() => 
    loadFromStorage(LS_KEYS.SELECTED_TOOLS, [], val => {
      try {
  ...
  const [functionCallingMode, setFunctionCallingMode] = useState(() => 
    loadFromStorage(LS_KEYS.FUNCTION_CALLING_MODE, DEFAULT_FUNCTION_CALLING_MODE));
  ...
  // Persist function calling settings
  useEffect(() => {
    saveToStorage(LS_KEYS.ENABLE_FUNCTION_CALLING, enableFunctionCalling, val => val.toString());
  }, [enableFunctionCalling]);

  useEffect(() => {
    saveToStorage(LS_KEYS.SELECTED_TOOLS, selectedTools, val => JSON.stringify(val));
  }, [selectedTools]);

  useEffect(() => {
    saveToStorage(LS_KEYS.FUNCTION_CALLING_MODE, functionCallingMode);
  }, [functionCallingMode]);
